...
.controller("DistrictsControllers", function($scope, $http, $q, $route){

    $scope.newDistricte = "-1";
    ...
    let defered = $q.defer();
    $http.post("models/districtes.php", data,
    ...
    $scope.getBarris = () => {
       ...
       if(!($scope.distID == -1)){
          $('#modifBarriCollapse').collapse('hide');
          let data = new FormData();
 
          data.append("acc", "r-b");
          data.append("d_id", $scope.distID);
    ...
    $scope.fillCollBarri = (id, nom) => {
       $scope.barriDist = $scope.distID;
       $scope.barriTitle = nom;
       $scope.barriID = id;
       $('#modifBarriCollapse').collapse('show');
    }
 
 
    $scope.hideBarriCollapse = () =>{
       $('#modifBarriCollapse').collapse('hide');
    }
 
    $scope.modifyBarri = (barriTitle, distID) =>{
       
       let data = new FormData();
 
       data.append("acc", "u-b");
       data.append("b_id", $scope.barriID);
       data.append("b_barri", barriTitle);
       data.append("b_idDist", distID);
    ...
    $scope.removeBarri = (id) => {
       if(confirm("Are you sure?")){
          ...
          .then((res) => {
             defered.resolve(res);
             if(res.data==1451){
                alert('Unable to remove barri, please remove its relations first.');
             } else if(res.data==1) {
                alert('Barri deleted successfully.');
             }
          })
          .catch((err) => { console.log(err.statusText) })
          .finally(() => {$scope.distID = -1});
       }
    }
 
    $scope.addBarri = (barriName, newBarriDist) => {
       ...
       data.append("acc", "c-b");
       data.append("barriName", barriName);
       data.append("newBarriDist", newBarriDist);
       let defered = $q.defer();
       $http.post("models/districtes.php", data,
          { headers:{ "Content-type" : undefined }, transformRequest : angular.identity })
       ...
       .finally(() => {
          $scope.distID = -1
          $('#newBarriModal').modal('hide');
       });
 
    }
 
    $scope.modalDist = (id, name) => {
       $scope.districteID = id;
       $scope.districteNom = name;
    ...
    $scope.delDist = (id) => {
       if(confirm("Are you sure?")){
          ...
          .then((res) => {
             defered.resolve(res);
             if(res.data==1451){
                alert('Unable to remove districte, please remove its relations first.');
             } else if(res.data==1) {
                alert('Districte deleted successfully.');
                $route.reload();
             }
          })
          .catch((err) => { console.log(err.statusText) })
    ...
    $scope.editDist = () =>{
       ...
       data.append("acc", "u-d");
       data.append("d_id", $scope.districteID);
       data.append("d_name", $scope.districteNom);
       let defered = $q.defer();
       $http.post("models/districtes.php", data,
          { headers:{ "Content-type" : undefined }, transformRequest : angular.identity })
       ...
       .then((res) => {
          ...
          if(res.data==1){
             ...
             $route.reload();
             alert('successfully updated dist.');
          }
       })
       .catch((err) => { console.log(err.statusText) })
       .finally(() => {
    ...
    $scope.createDist = () => {
       ...
       data.append("acc", "c-d");
       data.append("d_name", $scope.newDistricteNom);
       let defered = $q.defer();
       $http.post("models/districtes.php", data,
          { headers:{ "Content-type" : undefined }, transformRequest : angular.identity })
